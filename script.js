/* ==========================================================================
   ...
   Vanilla JS + GSAP/ScrollTrigger. No frameworks.
   ========================================================================== */
...
(() => {
  ...
  function resizeStarfield(){
    ...
    H = starCanvas.height = window.innerHeight * Math.max(document.body.scrollHeight / window.innerHeight, 1);
  ...
  function drawStarfield(t){
    ...
    if(!reduceMotion && t - lastShoot > (Math.random() * 3000 + 4500)){
      spawnShootingStar();
      lastShoot = t;
    }
  ...
  /* ============================================================
     2. CURSOR TRAIL — glowing star particles following pointer
     ============================================================ */
  ...
  function initReveals(){
    ...
    // admire cards stagger
    gsap.utils.toArray('.admire-grid .admire-card').forEach((card, i) => {
      gsap.to(card, {
  ...
  function initProgressRail(){
    ...
    ScrollTrigger.create({
      ...
      onUpdate: self => { fillEl.style.height = (self.progress * 100) + '%'; }
    });
  }

  /* ============================================================
     5. HERO — begin journey button, mouse parallax on glow
     ============================================================ */
  ...
  window.addEventListener('pointermove', (e) => {
    if(reduceMotion || !heroGlow) return;
    const x = (e.clientX / window.innerWidth - 0.5) * 30;
    const y = (e.clientY / window.innerHeight - 0.5) * 30;
    gsap.to(heroGlow, { x, y, duration: 1.2, ease: 'power2.out' });
  });
  ...
  /* ============================================================
     8. FLOWERS — SVG illustrations + bloom on scroll + petals
     ============================================================ */
  const flowerSVGs = {
    rose: `
  ...
  document.querySelectorAll('.flower-svg').forEach(svg => {
    ...
    ScrollTrigger.create({
      trigger: svg, start: 'top 82%',
      onEnter: () => gsap.to(bloom, { scale: 1, opacity: 1, duration: 1.4, ease: 'back.out(1.6)' })
    });
  });

  // gentle petal drift within the flowers section
  const petalField = document.getElementById('petalField');
  const petalColors = ['#F472B6', '#f7a8cd', '#FACC15'];
  if(!reduceMotion){
    function spawnPetal(){
      if(!isInViewport(document.getElementById('flowers'))) return;
      ...
      gsap.to(p, {
        y: 420, x: (Math.random() - 0.5) * 160, rotate: Math.random() * 360,
        opacity: 0, duration, ease: 'sine.inOut',
        onComplete: () => p.remove()
      });
    }
  ...
  /* ============================================================
     9. CAKE — flicker already CSS driven; wish button blows candles
     ============================================================ */
  ...
  function spawnSmoke(){
    ...
    candles.forEach(c => {
      for(let i=0;i<5;i++){
        ...
        c.appendChild(s);
        gsap.fromTo(s,
          { y:0, x:0, opacity:.6, scale:.5 },
          { y:-40 - Math.random()*20, x:(Math.random()-0.5)*30, opacity:0, scale:2, duration:1.6, delay:i*0.08, ease:'sine.out', onComplete:()=>s.remove() }
        );
      }
    });
  }
  ...
  function nextBirthday(){
    ...
    let target = new Date(year, 7, 1, 0, 0, 0); // Aug is month index 7
    if(now > target){
      // if today IS Aug 1, treat as arrived; otherwise roll to next year
  ...
  function tickCountdown(){
    ...
    if(isBirthdayToday){
      ...
      countdownTitle.textContent = "It's Her Day";
      return;
    }
  ...
  /* ============================================================
     11. ENDING — canvas writes "SRABONI" with gathering stars
     ============================================================ */
  ...
  ScrollTrigger.create({
    trigger: '#ending', start: 'top 70%',
    onEnter: animateName
  });

  /* ============================================================
     12. CONFETTI + FIREWORKS (canvas-based, no external lib needed)
     ============================================================ */
